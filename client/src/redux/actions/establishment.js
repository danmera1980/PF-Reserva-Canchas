...
export function postEstablishment(payload, userToken) {
  ...
  return async function () {
    var establishment = await axios.post(
      `${SERVER_URL}/establishment`,
      payload,
      { headers: headers }
    );
...
export const addUserToEstablishment = (payload) => {
  return async function () {
    var info = await axios.post(
      `${SERVER_URL}/establishment/addUserToEstablishment`,
      payload
    );
    return info;
  };
...
export const searchByText = (searchText) => {
  ...
  return async (dispatch) => {
    var results = await axios(
      `${SERVER_URL}/findlocation?latitude=${latitude}&longitude=${longitude}&zoom=${zoom}&sport=${sport}`
    );
    return dispatch({
      type: SEARCH_TEXT,
...
export const getGeocode = (searchText) => {
  return async (dispatch) => {
    if (searchText !== "") {
      var results = await axios(
        `https://api.mapbox.com/geocoding/v5/mapbox.places/${searchText}.json?access_token=${mapToken}`
      );
    } else {
      var results = { data: "" };
    }
